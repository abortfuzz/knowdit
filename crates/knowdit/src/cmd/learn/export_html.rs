...
impl ExportHtmlArgs {
    pub async fn run(self, db: &HistoricalDatabase) -> Result<()> {
        ...
        let project_rows = self.project_rows.unwrap_or_else(|| {
            auto_project_rows(kg.projects.len(), self.semantic_rows, self.finding_rows)
        });
        let paths = export_paths(self.output);
        ...
        println!("Semantic rows set to {}", self.semantic_rows);
        println!("Finding rows set to {}", self.finding_rows);
